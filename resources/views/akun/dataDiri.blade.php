...
<div class="bg-white p-3 rounded border flex justify-between">
  ...
  <div>
    ...
    @elseif (Auth::user()->segmen == "proses_baru" || Auth::user()->segmen == "proses_perpanjang")
      <div class="capitalize font-bold text-rose-500 px-2 italic"><a href="{{ route('akun.memberBayar') }}" class="bg-sky-500 ml-2 rounded-full py-1 px-3 text-white ring-offset-1 ring-1 ring-sky-500">Klik Disini</a> untuk detail proses member</div>
    @elseif (Auth::user()->segmen == "proses_lama")
      <div class="capitalize font-bold text-rose-500 px-2 italic">Proses member sedang dicek</div>      
    @else
...
<script>
  ...
  if (isMobileDevice) {
    window.location.href = "{{ route('mAkun') }}";
  }
</script>
